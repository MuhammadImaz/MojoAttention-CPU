...
`validate_and_dispatch` accepts an injected synchronous dispatch callable. Rejected inputs produce zero calls and leave all tensors unchanged. A valid request receives exactly one call using the original tensor objects. Under inference mode, the caller-owned output is marked unwritten with a reserved float32 NaN bit pattern; after return, only unchanged sentinel bits produce `KERNEL-INCOMPLETE-WRITE`, so a fully written nonfinite result is not mislabeled as incomplete. Story 2.4 supplies the real Mojo dispatch implementation and native exception translation.
